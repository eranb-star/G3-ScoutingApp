...
- Final website release: source `f2c3f416e23729ec12348c82f563fed56663913e`, Vercel `6TZsuSvJKCpKcmkNGTfH8unQ13oH`, Ready/Production with `g3-6740.com` alias, production-environment rebuild completed at 15:24 Israel time. Main implementation equivalent `45b6a2c`. Backend bundle above is unchanged. Documentation-only commits after this source do not imply a different running website.
- Final authenticated production reload verified corrected reviewed-policy wording, official limits/citations, permitted-mount checkbox and exclusion from camera recommendations. Unchecking Mount 1 removed it from the recommendation; rechecking restored it. Saved private test plan revision3 with waypoint2 X=-4; list immediately showed v3 and history offered v3/v2/v1. No captured browser console errors in this acceptance. The named private verification plan remains in the account.
- Permitted locations are manually marked candidates, not automatically inferred safe mounting surfaces. The checkbox is applied to both fast proxy recommendations and actual-CAD analysis. Local regression covers excluded candidates.
- Rollback: previous website `BGCnXh9QXuPXJYEJibSTc9JsPaMV` remains available; earlier pre-engineering release `NefhtgnnVSdxYfQDPDRTYc3XbaRm`. Prefer a reviewed frontend rollback with additive schemas retained; do not drop plan/rule history to roll back UI. Rule/policy deactivation and prior-current-source revision activation use the administrator lifecycle rather than destructive SQL.
