...
- Historical failure: Hebrew prose overstated necessary alliance conditions despite correct sources; the initial prompt-only evaluation was blocked by daily allowance. The subsequent ENGINEERING_RELEASE_20260924.md supersedes that open status for supported mixed-level scoring: deterministic EN/HE answers and production L1+L2+L2=50 acceptance passed. Broader prose evaluation remains ongoing quality assurance; do not describe the delivered correction as unfinished. Existing bad saved answers are not automatically rewritten.
