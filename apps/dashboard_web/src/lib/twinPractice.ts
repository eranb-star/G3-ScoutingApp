...
export class PracticeRun{
 ...
 finish(pose:Pose,complete=false){if(!this.active)return;this.active=false;if(this.steps%6!==0)this.save(pose,false);const scored=Math.max(0,(pose.scores??[]).reduce((a,b)=>a+b,0)-(this.initial.scores??[]).reduce((a,b)=>a+b,0)),shots=Math.max(0,(pose.shots??0)-(this.initial.shots??0));this.result={seconds:this.seconds,scored,shots,accuracy:shots?scored/shots:null,distance:Math.max(0,pose.distance-this.initial.distance),complete,input:this.input,settings:this.settings};}
 frame(seconds:number):PracticeFrame{
  ...
  return {seconds:time,intake:a.intake,pose:{...a.pose,x:lerp(a.pose.x,b.pose.x),y:lerp(a.pose.y,b.pose.y),z:lerp(a.pose.z??0,b.pose.z??0),heading,rotation,balls:a.pose.balls?.map(p=>{const next=balls.get(p.id);return next?{...p,x:lerp(p.x,next.x),y:lerp(p.y,next.y),z:lerp(p.z??0,next.z??0)}:p;})}};
 }
}
