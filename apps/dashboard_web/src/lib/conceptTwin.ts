...
export type Pose = {z?:number;rotation?:{x:number;y:number;z:number;w:number};contacts?:number;speed?:number;x:number;y:number;heading:number;tick:number;distance:number;collisions:number};
