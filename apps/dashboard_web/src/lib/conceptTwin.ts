...
export type Concept = {robotId?:'darwin'|'kitbot';length:number;width:number;height:number;speed:number;turn:number};
export type Command = {vx:number;vy:number;omega:number};
export type OpponentPose={x:number;y:number;z:number;heading:number;rotation:{x:number;y:number;z:number;w:number};state:string;intake:boolean;stored:number;scored:number};
export type Pose = {opponent?:OpponentPose;playerScored?:number;storedIds?:number[];fuelVisualEpoch?:number;fuelVisualEvents?:import('./fuelPhysics').FuelVisualEvent[];velocity?:{x:number;y:number;z:number};angularVelocity?:{x:number;y:number;z:number};ledger?:{field:number;stored:number;hub:number;outposts:number;outOfPlay:number;total:number};scores?:number[];shots?:number;balls?:import('./intake').BallPose[];collected?:number;z?:number;rotation?:{x:number;y:number;z:number;w:number};contacts?:number;speed?:number;x:number;y:number;heading:number;tick:number;distance:number;collisions:number};
