...
export type Pose = {velocity?:{x:number;y:number;z:number};angularVelocity?:{x:number;y:number;z:number};ledger?:{field:number;stored:number;hub:number;outposts:number;outOfPlay:number;total:number};scores?:number[];shots?:number;balls?:import('./intake').BallPose[];collected?:number;z?:number;rotation?:{x:number;y:number;z:number;w:number};contacts?:number;speed?:number;x:number;y:number;heading:number;tick:number;distance:number;collisions:number};
