...
export type PlanningDraft={schema:1;status:'unverified-planning-draft';assetKey?:string;season:SeasonPackage;robot:MotionProfile;route:RoutePoint[];cameras:CameraMount[]};
export function parsePlanningDraft(text:string):PlanningDraft{
 ...
 if(d.assetKey!==undefined&&(typeof d.assetKey!=='string'||!d.assetKey||d.assetKey.length>240))throw Error('Invalid robot asset identity');
 ...
 return {schema:1,status:'unverified-planning-draft',...(d.assetKey?{assetKey:d.assetKey}:{}),season,robot:structuredClone(d.robot),route:structuredClone(d.route),cameras:structuredClone(d.cameras)};
}
