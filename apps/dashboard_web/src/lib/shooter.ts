export type ShooterConfig={on:boolean;speed:number;elevation:number;rate:number;height:number;yaw?:number;lanes?:number};
export const DEFAULT_SHOOTER:ShooterConfig={on:false,lanes:1,speed:5.6,elevation:65,rate:2,height:0.8};
...
export function muzzleOffset(q:Quaternion,length:number,height:number,yaw=0,lateral=0){return rotateVector(q,(length/2+.09)*Math.cos(yaw*Math.PI/180)-lateral*Math.sin(yaw*Math.PI/180),(length/2+.09)*Math.sin(yaw*Math.PI/180)+lateral*Math.cos(yaw*Math.PI/180),Math.max(.3,Math.min(1.5,height))-.3);}
