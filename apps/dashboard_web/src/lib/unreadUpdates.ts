...
export async function getUnreadUpdateCounts(memberId:string){
  const [{data:member},{data:announcements},{data:announcementReads},{data:messages},{data:channelReads}]=await Promise.all([
    supabase.from("team_members").select("role,subteam").eq("id",memberId).maybeSingle(),
  ...
  const subteam=String(member?.subteam??"").trim().toLowerCase();
  const visibleAnnouncements=(announcements??[]).filter(item=>{
    ...
    if(audience==="members")return role==="member"||role==="student";
    if(audience==="subteam")return Boolean(subteam)&&String(item.audience_subteam??"").trim().toLowerCase()===subteam;
    return false;
  });
