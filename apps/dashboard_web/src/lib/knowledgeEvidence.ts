...
export function findEvidence(claims:EvidenceClaim[],sources:EvidenceSource[],season:number|'all',query:string){
 const words=query.toLowerCase().trim().split(/\s+/).filter(Boolean);
 const allowed=new Set(sources.filter(s=>s.current&&(season==='all'||s.season===season)).map(s=>s.id));
