...
export function geometry(p:Pose,c:Concept,s:ShooterConfig,target:0|1){
 ...
 const offset=muzzleOffset(q,c.length,s.height),muzzle={x:p.x+offset.x,y:p.y+offset.y,z:(p.z??0)+.3+offset.z};
 ...
 const forward=rotateVector(q,1,0,0),left=rotateVector(q,0,1,0);
 ...
 return {tick:p.tick,simSeconds:p.tick*DT,x:p.x,y:p.y,z:p.z??0,headingDeg:wrapDegrees(p.heading),
  ...
  aimErrorDeg:Math.hypot(dx,dy)<1e-8?null:wrapDegrees(Math.atan2(dy,dx)-Math.atan2(forward.y,forward.x)),
