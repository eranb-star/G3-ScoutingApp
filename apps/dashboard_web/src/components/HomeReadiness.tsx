...
export function HomeTeamRisks({data,error}:{data:ReadinessData|null;error:boolean}){
 ...
 return <section className="hub-card home-team-risks"><header><h2>{pick('Team readiness','מוכנות הקבוצה')}</h2>{data&&!error?<small>{pick('Updated','עודכן')} {new Date(data.as_of).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</small>:null}</header>
 ...
 <small>{pick('Checks cover critical robot issues and inventory items with stock monitoring enabled.','הבדיקות כוללות תקלות קריטיות ברובוט ופריטים שמעקב המלאי שלהם מופעל.')}</small>
 </>}
 </section>;
}
