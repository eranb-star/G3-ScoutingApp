...
export default function PlanningField({season,children,active}:{season:SeasonPackage;children:ReactNode;active:boolean}){
 ...
 const draw=()=>{if(renderer&&!disposed&&element.clientWidth){renderer.setSize(element.clientWidth,element.clientHeight,false);renderer.render(scene,camera);}};const observer=new ResizeObserver(draw);observer.observe(element);
