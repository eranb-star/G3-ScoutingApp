...
export default function HomeActionInbox({mode="home",overdueTasks=[],risks=[]}:{mode?:"home"|"work";overdueTasks?:OverdueTask[];risks?:ReadinessSignal[]}) {
 ...
 useProjectRefresh(()=>load());
  ...
  async function updateState(action:Action,status:string,snoozed_until?:string){if(!profile||status==="completed"&&['project_tasks','project_task_collaborators'].includes(action.source_table??''))return;const now=new Date().toISOString();const {error}=await supabase.from("team_action_states").upsert({action_id:action.id,member_id:profile.id,status,snoozed_until:snoozed_until??null,acknowledged_at:status==="acknowledged"?now:null,completed_at:status==="completed"?now:null,updated_at:now},{onConflict:"action_id,member_id"});setMessage(error?.message??pick("Reminder updated. Task status is unchanged.","התזכורת עודכנה. סטטוס המשימה לא השתנה."));if(!error)await load();}
