...
export type NumericRule={unit:string;minimum:string|number|null;maximum:string|number|null;tolerance:string|number|null};
export type MilestoneRequirement={requirement:string;acceptance:string;method:string;numeric_rule?:NumericRule|null};
...
export function ReviewRequirements({items,onChange,disabled}:{items:MilestoneRequirement[];onChange:(items:MilestoneRequirement[])=>void;disabled:boolean}){
 ...
 function update(i:number,key:keyof MilestoneRequirement,value:string){onChange(items.map((r,index)=>index===i?{...r,[key]:value,...(key==='method'&&!['test','demonstration'].includes(value)?{numeric_rule:null}:{})}:r));}
 function setRule(i:number,rule:NumericRule|null){onChange(items.map((r,index)=>index===i?{...r,numeric_rule:rule}:r));}
 ...
 {['test','demonstration'].includes(r.method)?<label>{pick('Numeric acceptance range','טווח קבלה מספרי')}<select value={r.numeric_rule?'range':'written'} onChange={e=>setRule(i,e.target.value==='range'?{unit:'',minimum:'',maximum:'',tolerance:0}:null)}><option value="written">{pick('Written criterion','תנאי כתוב')}</option><option value="range">{pick('Measured range with tolerance','טווח נמדד עם סבילות')}</option></select></label>:null}
 {r.numeric_rule?<><label>{pick('Canonical unit — findings must use this unit','יחידה קנונית — יש לרשום ממצאים ביחידה זו')}<input required maxLength={40} placeholder="mm / N / V" value={r.numeric_rule.unit} onChange={e=>setRule(i,{...r.numeric_rule!,unit:e.target.value})}/></label>{([['minimum','Minimum (optional)','מינימום (רשות)'],['maximum','Maximum (optional)','מקסימום (רשות)'],['tolerance','Allowed tolerance','סבילות מותרת']] as const).map(([field,en,he])=><label key={field}>{pick(en,he)}<input type="number" step="any" required={field==='tolerance'||(field==='minimum'&&!String(r.numeric_rule?.maximum??''))} min={field==='tolerance'?0:undefined} value={r.numeric_rule?.[field]??''} onChange={e=>setRule(i,{...r.numeric_rule!,[field]:e.target.value})}/></label>)}</>:null}
