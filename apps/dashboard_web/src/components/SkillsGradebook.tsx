...
export default function SkillsGradebook({pick,profileId,canReview,courses,modules,enrollments,evidence,assessments,submissions,people,progressEvents}:Props){
  ...
  const attention=summary.filter(row=>row.status==="overdue"||row.status==="changes_requested"||row.exhausted);
  ...
  return <section className="skills-gradebook">
    ...
    {canReview&&attention.length?<section className="gradebook-attention" aria-labelledby="learning-attention-title"><header><div><span>{pick("Support queue","תור תמיכה")}</span><h3 id="learning-attention-title">{pick("Members who may be blocked","חברים שעשויים להיות חסומים")}</h3></div><b>{attention.length}</b></header><div>{attention.slice(0,6).map(row=><button type="button" key={row.enrollment.id} onClick={()=>setExpanded(row.enrollment.id)}><span><strong>{row.person?.display_name}</strong><small>{row.course?.title}</small></span><em>{row.exhausted?pick("Attempts exhausted","הניסיונות הסתיימו"):label(row.status)}</em></button>)}</div></section>:null}
    ...
    <div className="gradebook-list">{visible.length?visible.map(row=><article className={`gradebook-row status-${row.status}`} key={row.enrollment.id}><button type="button" className="gradebook-row-main" onClick={()=>setExpanded(expanded===row.enrollment.id?null:row.enrollment.id)} aria-expanded={expanded===row.enrollment.id}><span className="gradebook-person"><b>{canReview?row.person?.display_name:row.course?.title}</b><small>{canReview?row.course?.title:row.course?.domain}</small></span><span className={`gradebook-status status-${row.status}`}>{row.exhausted?pick("Blocked · no attempts","חסום · אין ניסיונות"):label(row.status)}</span><span className="gradebook-progress"><i><em style={{width:`${row.percent}%`}}/></i><b>{row.percent}%</b></span><span className="gradebook-counts"><small>{pick("Modules","יחידות")}</small><b>{row.approved}/{row.moduleTotal}</b></span><span className="gradebook-counts"><small>{pick("Assessments","מטלות")}</small><b>{row.passedCount}/{row.assessmentTotal}</b></span><span className="gradebook-expand">{expanded===row.enrollment.id?"−":"+"}</span></button>{expanded===row.enrollment.id?<div className="gradebook-detail"><div><strong>{pick("Assessment history","היסטוריית מטלות")}</strong>{assessments.filter(item=>item.course_id===row.enrollment.course_id).map(assessment=>{const attempts=row.attempts.filter(item=>item.assessment_id===assessment.id);return <section key={assessment.id}><header><b>{assessment.title}</b><span>{attempts.length}/{assessment.max_attempts} {pick("attempts","ניסיונות")}</span></header>{attempts.length?attempts.map(attempt=><div className="attempt-line" key={attempt.id}><span>#{attempt.attempt_number} · {attempt.status.replaceAll("_"," ")}</span><b>{attempt.score!=null?`${attempt.score}/${assessment.max_score}`:pick("No score","ללא ציון")}</b><small>{attempt.submitted_at?new Date(attempt.submitted_at).toLocaleString():""}</small>{attempt.response?<p><strong>{pick("Response","תשובה")}:</strong> {attempt.response}</p>:null}{Object.values(attempt.answers??{}).length?<p><strong>{pick("Answers","תשובות")}:</strong> {Object.values(attempt.answers).map(value=>Array.isArray(value)?value.join(" + "):value).join(" · ")}</p>:null}{attempt.feedback?<p><strong>{pick("Feedback","משוב")}:</strong> {attempt.feedback}</p>:null}</div>):<small>{pick("No submission yet","אין עדיין הגשה")}</small>}</section>})}<section className="gradebook-timeline"><header><b>{pick("Progress timeline","ציר התקדמות")}</b></header>{progressEvents.filter(event=>event.enrollment_id===row.enrollment.id).slice(0,8).map(event=><div key={event.id}><i aria-hidden="true"/><span><b>{event.event_type.replaceAll("_"," ")}</b><small>{new Date(event.created_at).toLocaleString()}</small>{event.details?<em>{event.details}</em>:null}</span></div>)}</section></div><aside><strong>{pick("Course requirements","דרישות הקורס")}</strong><span>{row.approved}/{row.moduleTotal} {pick("modules approved","יחידות אושרו")}</span><span>{row.passedCount}/{row.assessmentTotal} {pick("required assessments passed","מטלות חובה עברו")}</span>{row.enrollment.due_at?<span>{pick("Course due","מועד הקורס")}: {new Date(`${row.enrollment.due_at}T12:00:00`).toLocaleDateString()}</span>:null}</aside></div>:null}</article>):<div className="academy-empty"><strong>{pick("No matching learning records","לא נמצאו רשומות למידה")}</strong><p>{pick("Change the filters or assign a course to begin tracking progress.","שנו את המסננים או הקצו קורס כדי להתחיל לעקוב אחר ההתקדמות.")}</p></div>}</div>
  </section>;
}
