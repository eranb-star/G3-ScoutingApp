...
export default function MentorReviewQueue({includePlanned=false}:{includePlanned?:boolean}){
 ...
 async function load(){if(!eligible)return;const r=await supabase.rpc('project_review_context');const planned=includePlanned?await supabase.rpc('planned_mentor_reviews'):{data:[],error:null};setError(!!r.error||!!planned.error);setItems([...(r.data??[]),...(planned.data??[])].filter((g:ReviewGate)=>needsReview(g,profile?.id??'',includePlanned)));}
