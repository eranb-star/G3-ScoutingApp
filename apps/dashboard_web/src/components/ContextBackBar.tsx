import {subscribeActiveEvent} from "../lib/activeEvent";
...
export default function ContextBackBar({fallbackTo,label}:{fallbackTo?:string;label?:string}={}) {
  ...
  const [eventState,setEventState]=useState<"empty"|"loading"|"ready"|"error">("empty");
  ...
  useEffect(()=>{
    let generation=0;
    const refresh=()=>{const ticket=++generation;if(!showEventContext){setEventName("");setEventState("empty");return;}const id=localStorage.getItem("g3_event_id");if(!id){setEventName("");setEventState("empty");return;}setEventState("loading");void supabase.from("events").select("name").eq("id",id).maybeSingle().then(({data,error})=>{if(ticket!==generation)return;setEventName(data?.name??"");setEventState(error||!data?"error":"ready");});};
    const unsubscribe=subscribeActiveEvent(refresh);refresh();return()=>{generation++;unsubscribe();};
  },[showEventContext,location.pathname]);
