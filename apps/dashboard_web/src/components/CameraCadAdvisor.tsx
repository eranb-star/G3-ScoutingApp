...
export default function CameraCadAdvisor(props:Props){
 ...
 async function analyze(){if(!loaded)return;const epoch=++generation.current;setBusy(true);setProgress(0);setRows([]);setStatus('');
  try{const stride=Math.max(1,Math.ceil(props.samples.length/80)),samples=props.samples.filter((_,i)=>i%stride===0||i===props.samples.length-1),geometry=meshVisibility(loaded.field,loaded.robot),coverage:number[][][]=[];
   for(let c=0;c<props.cameras.length;c++){const values:number[][]=[];for(let i=0;i<samples.length;i++){if(epoch!==generation.current)return;values.push(cameraCoverage(props.season,props.cameras[c],[samples[i]],[],geometry)[0].tagIds);if(i%4===0){setProgress(Math.round(100*(c*samples.length+i+1)/(props.cameras.length*samples.length)));await new Promise<void>(resolve=>setTimeout(resolve,0));}}coverage.push(values);}
   if(epoch===generation.current){setRows(compareCoverageSets(props.cameras.map(c=>c.id),coverage,props.maxCameras,target));setProgress(100);}
