...
export default function TrainingCenterPage(){
  ...
  async function enroll(){const ids=assignmentMode==="all"?people.map(x=>x.id):assignmentMode==="subteam"?people.filter(x=>memberTeams(x).some(team=>normalizedTeamMatch(team,assignSubteam))).map(x=>x.id):assignMembers;if(!selected||!ids.length){setMessage(pick("Choose at least one member or subteam.","בחרו לפחות חבר/ה או תת־צוות."));return;}const {error}=await supabase.from("training_enrollments").upsert(ids.map(member_id=>({course_id:selected,member_id,assigned_by:profile?.id,due_at:assignDue||null})),{onConflict:"course_id,member_id"});setMessage(error?.message??pick(`Course assigned to ${ids.length} members.`,`הקורס הוקצה ל־${ids.length} חברים.`));if(!error){setAssignMembers([]);await load();}}
