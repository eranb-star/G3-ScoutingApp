...
export default function UpdatesPage() {
  ...
  function canSeeAnnouncement(item:Announcement){
    ...
    if(target==="mentors")return role==="mentor"||role==="admin";
    if(target==="members")return role==="member"||role==="student"||role==="team_leader";
    if(target==="subteam")return memberTeams(profile).some(team=>normalizedTeamMatch(team,item.audience_subteam));
    return false;
  }
  ...
  async function loadCore() {
    ...
    const [announcementResult, readResult, channelResult, savedResult] = await Promise.all([
      supabase.from("announcements").select("id,title,body,priority,published_at,archived,audience,audience_subteam").eq("archived", isAdmin ? showArchived : false).order("published_at", { ascending: false }),
    ...
    setAnnouncements(((announcementResult.data ?? []) as Announcement[]).filter(canSeeAnnouncement));
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      const counts = await getUnreadUpdateCounts(profile.id);
      if (alive) {
        setUnreadTotal(counts.announcements + counts.channels + counts.actions);
        setUnseenChannelCount(counts.channels);
      }
    };
    const events = ["g3-actions-changed", "g3-announcements-changed", "g3-channels-seen", "focus"];
  ...
  useEffect(()=>{
    ...
    if(next&&["inbox","announcements","channels","knowledge"].includes(next))setView(next);
    const announcementId=params.get("announcement");
    if(announcementId)requestAnimationFrame(()=>document.getElementById(`announcement-${announcementId}`)?.scrollIntoView({behavior:"smooth",block:"center"}));
  },[params]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    void (async()=>{await supabase.from("channel_read_state").upsert({member_id:profile.id,channel_id:selectedChannel,last_read_at:new Date().toISOString()});const counts=await getUnreadUpdateCounts(profile.id);setUnseenChannelCount(counts.channels);setUnreadTotal(counts.announcements+counts.channels+counts.actions);window.dispatchEvent(new Event("g3-channels-seen"));})();
  ...
  async function publishAnnouncement(event: FormEvent) {
    ...
    if (!profile) return;
    const { data, error } = await supabase.from("announcements").insert({ title: announcementTitle.trim(), body: announcementBody.trim(), priority: announcementPriority, audience, audience_subteam:audience==="subteam"?audienceSubteam:null, created_by: profile.id }).select("id").single();
  ...
  const tabs: Array<[View,string,string,number?]> = [
    ["inbox", "Inbox", "דואר נכנס", unreadTotal], ["announcements", "Announcements", "הודעות"], ["channels", "Channels", "ערוצים", unseenChannelCount], ["knowledge", "FRC Knowledge", "ידע FRC"],
  ];
