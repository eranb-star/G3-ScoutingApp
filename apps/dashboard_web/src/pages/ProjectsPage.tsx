...
export default function ProjectsPage() {
 ...
 useProjectRefresh(()=>load());
  ...
  async function createTask(event:FormEvent) {
    ...
    const { error } = await supabase.rpc("create_task_with_collaborators",{p_project:taskProject,p_title:taskTitle.trim(),p_due:taskDue?new Date(`${taskDue}T18:00:00+03:00`).toISOString():null,p_owner:taskOwner,p_members:taskMembers.filter(id=>id!==taskOwner)});
    setCreatingTask(false);
    setMessage(error?.message ?? pick("Task created.", "המשימה נוצרה."));
    if (!error) { sessionStorage.removeItem("g3-project-task-draft");setAssistantDraft(false);setTaskProject(null); setTaskMembers([]); setTaskTitle(""); setTaskDue(""); await load();notifyProjectChange(); }
  }
