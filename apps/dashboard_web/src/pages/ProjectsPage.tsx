...
export default function ProjectsPage() {
 ...
 useProjectRefresh(()=>load());
  ...
  async function offerArchiveTask(task:Task) {
    const explanation=pick("This task has review records or linked work, so permanent deletion is protected. Archiving removes it from active work and preserves its history.","למשימה יש רשומות ביקורת או עבודה מקושרת ולכן המחיקה לצמיתות מוגנת. העברה לארכיון מסירה אותה מהעבודה הפעילה ושומרת את ההיסטוריה.");
    setMessage(explanation);
    if(!task.archived)setArchiveOffer(task.id);
  }
  async function removeTask(task:Task) {
    if(!isAdmin)return;
    if(reviews.some(g=>g.task_id===task.id)){await offerArchiveTask(task);return;}
    if(!confirm(pick(`Delete “${task.title}” permanently?`,`למחוק לצמיתות את “${task.title}”?`)))return;
    const {error}=await supabase.from("project_tasks").delete().eq("id",task.id).select("id").single();
    if(error?.code==="23503"){await offerArchiveTask(task);return;}
    setMessage(error?.message??pick("Task deleted.","המשימה נמחקה."));if(!error){await load();notifyProjectChange();}
  }
