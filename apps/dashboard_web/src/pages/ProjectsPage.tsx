import TaskAssistContext from '../components/TaskAssistContext';
import {assistDraftKey,parseAssistTaskDraft,type AssistTaskDraft} from '../lib/assistDecision';
import TaskCollaborators, {CollaboratorPicker} from "../components/TaskCollaborators";
...
export default function ProjectsPage() {
  ...
  const [assistTaskIds,setAssistTaskIds]=useState<Set<string>>(new Set());
 ...
 useProjectRefresh(()=>load());
  ...
  const [assistContext,setAssistContext]=useState<AssistTaskDraft|null>(null);
  ...
  async function load() {
    ...
    const [projectResult, taskResult] = await Promise.all([
      ...
      supabase.from("project_tasks").select("id,project_id,title,status,due_at,archived,assignee_id,created_by,project_task_assist_context(task_id)").order("created_at"),
    ]);
    ...
    setAssistTaskIds(new Set((taskResult.data??[]).filter(t=>Array.isArray(t.project_task_assist_context)?t.project_task_assist_context.length>0:!!t.project_task_assist_context).map(t=>t.id)));
  }
  useEffect(() => { void load(); }, []);
  useEffect(()=>{if(!profile||params.get('assistantDraft')!=='1')return;try{const draft=parseAssistTaskDraft(sessionStorage.getItem(assistDraftKey(profile.id)),profile.id);if(draft){setAssistContext(draft);setTaskTitle(draft.title);setAssistantDraft(true);}}catch{setMessage(pick('Could not read the saved task draft.','לא ניתן לקרוא את טיוטת המשימה.'));}},[profile?.id]);
  ...
  async function createTask(event:FormEvent) {
    event.preventDefault(); if (!profile || !taskProject || !taskOwner || creatingTask) return;
    setCreatingTask(true);
    const args={p_project:taskProject,p_title:taskTitle.trim(),p_due:taskDue?new Date(`${taskDue}T18:00:00+03:00`).toISOString():null,p_owner:taskOwner,p_members:taskMembers.filter(id=>id!==taskOwner)};
    let outcome;try{outcome=await supabase.rpc(assistContext?'create_task_from_assist':'create_task_with_collaborators',assistContext?{...args,p_message:assistContext.message,p_request:assistContext.request}:args);}catch{setCreatingTask(false);setMessage(pick('Connection interrupted. Your draft is retained; retry uses the same request.','החיבור נותק. הטיוטה נשמרה; ניסיון חוזר משתמש באותה בקשה.'));return;}
    const {error,data:createdTask}=outcome;
    setCreatingTask(false);
    setMessage(error?.message ?? pick("Task created.", "המשימה נוצרה."));
    if (!error) { if(assistContext){try{sessionStorage.removeItem(assistDraftKey(profile.id));}catch{/* Saved task remains durable if browser storage is unavailable. */}setAssistContext(null);setParams({project:taskProject,task:createdTask});}setAssistantDraft(false);setTaskProject(null); setTaskMembers([]); setTaskTitle(""); setTaskDue(""); await load();notifyProjectChange(); }
  }
  ...
  return <main className="hub-page projects-page"><ProjectChangeImpact/>
    ...
    {assistantDraft?<div className="workspace-guidance assistant-task-draft"><span>✦</span><div><strong>{pick("G3 Assist task draft is ready","טיוטת משימה מ-G3 Assist מוכנה")}</strong><p>{pick("Choose a project and task owner. Saving shares this answer, question and sources with people who can access the task. It does not submit a mentor review or mark a test passed.","בחרו פרויקט ואחראי/ת. השמירה תשתף את השאלה, התשובה והמקורות עם מי שיכול לגשת למשימה. היא אינה מגישה ביקורת מנטור או מסמנת בדיקה שעברה.")}</p></div><button onClick={()=>{if(profile)try{sessionStorage.removeItem(assistDraftKey(profile.id));}catch{/* Saved task remains durable if browser storage is unavailable. */}setAssistContext(null);setAssistantDraft(false);setTaskTitle('');}}>×</button></div>:null}
