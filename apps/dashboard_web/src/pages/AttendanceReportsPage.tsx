...
type Meeting = { id:string; title:string; ends_at:string; status:string };
...
export default function AttendanceReportsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from("team_meetings").select("id,title,ends_at,status").gte("ends_at",from.toISOString()).lte("ends_at",new Date().toISOString()).neq("status","cancelled"),
  ...
  const expectedMeetings=Object.keys(meetings).length;
  const summary=useMemo(()=>Object.values(members).map(member=>{const memberRecords=records.filter(row=>row.member_id===member.id);return {member,records:memberRecords,sessions:memberRecords.length,hours:memberRecords.reduce((sum,row)=>sum+hours(row),0),rate:expectedMeetings?Math.min(100,(memberRecords.length/expectedMeetings)*100):0};}).sort((a,b)=>b.hours-a.hours||a.member.display_name.localeCompare(b.member.display_name)),[records,members,expectedMeetings]);
  ...
  function exportCsv(){const lines=[["Member","Subteam","Sessions","Hours","Attendance rate"],...summary.map(item=>[item.member.display_name,item.member.subteam??"",String(item.sessions),item.hours.toFixed(2),`${item.rate.toFixed(0)}%`])];const csv=lines.map(row=>row.map(value=>`"${value.replace(/"/g,'""')}"`).join(",")).join("\n");const link=document.createElement("a");link.href=URL.createObjectURL(new Blob([csv],{type:"text/csv"}));link.download=`g3-attendance-${period}.csv`;link.click();URL.revokeObjectURL(link.href);}
