...
import EventRobotChecks from "../components/EventRobotChecks";
import {serviceIsDue} from "../lib/operationsPlanning";
import HomeDependencyBlockers from "../components/HomeDependencyBlockers";
...
import {useHomeReadiness,HomeEventContext,HomeTeamRisks} from "../components/HomeReadiness";
...
export default function ProductivityHomePage({isAdmin,allowCheckIn=true}:{isAdmin:boolean;allowCheckIn?:boolean}){
  void isAdmin; const readiness=useHomeReadiness();
 ...
 useProjectRefresh(()=>load());
  async function load(){if(!profile)return;const now=new Date();const nowIso=now.toISOString();const [activeResult,nextResult,actionResult,stateResult,presenceResult,blockerProjects,blockerTasks,maintenanceResult,counts]=await Promise.all([supabase.from("team_meetings").select("id,title,starts_at,ends_at,status").eq("status","open").order("starts_at"),supabase.from("team_meetings").select("id,title,starts_at,ends_at,status").in("status",["scheduled","open"]).gt("starts_at",nowIso).order("starts_at").limit(1),supabase.from("team_actions").select("id,action_type,due_at,priority,source_table"),supabase.from("team_action_states").select("action_id,status,snoozed_until").eq("member_id",profile.id),supabase.from("attendance_records").select("id",{count:"exact",head:true}).is("checked_out_at",null),supabase.from("team_projects").select("id",{count:"exact",head:true}).eq("status","blocked"),supabase.from("project_tasks").select("id,team_projects(status)").eq("status","blocked").eq("archived",false),supabase.from("tool_maintenance").select("id",{count:"exact",head:true}).neq("status","resolved"),getUnreadUpdateCounts(profile.id)]);const openMeetings=(activeResult.data??[]) as Meeting[];const validBlockedTasks=((blockerTasks.data??[]) as unknown as Array<{team_projects?:{status?:string}|null}>).filter(task=>task.team_projects?.status!=="archived");setActive(openMeetings.find(meeting=>isUsableOpenMeeting(meeting,now))??null);setNext((nextResult.data?.[0]??null) as Meeting|null);setResponsibilityCount(visibleResponsibilities((actionResult.data??[]) as Array<{id:string;action_type:string;due_at:string|null;priority:string}>,(stateResult.data??[]) as Array<{action_id:string;status:string;snoozed_until:string|null}>,"work").length);setPresence(presenceResult.count??0);setBlockers((blockerProjects.count??0)+validBlockedTasks.length);setMaintenance(maintenanceResult.count??0);setUnread(counts.announcements+counts.channels+counts.actions);}
  useEffect(()=>{void load();},[profile?.id]);
  useEffect(()=>{if(!profile)return;void Promise.all([supabase.from("robot_components").select("id,status,service_interval_days,last_serviced_at,first_service_due").in("status",["installed","service_due","failed"]),supabase.from("robot_batteries").select("id",{count:"exact",head:true}).in("status",["service_due","quarantined"])]).then(([components,batteries])=>{const componentAlerts=(components.data??[]).filter(item=>item.status==="failed"||serviceIsDue(item)).length;setRobotMaintenance(componentAlerts+(batteries.count??0));});},[profile?.id]);
  return <main className="hub-page productivity-home"><section className="home-command"><div><div className="hub-eyebrow">Glue Gun &amp; Glitter · FRC 6740</div><h1>{pick(`Ready, ${profile?.display_name?.split(" ")[0]??"G3"}?`,`מוכנים, ${profile?.display_name?.split(" ")[0]??"G3"}?`)}</h1><HomeEventContext data={readiness.data} error={readiness.error}/></div><img src="/logoG3.png" alt="G3 6740"/></section>
  ...
  <div className="home-workspace"><div className="home-primary"><HomeActionInbox overdueTasks={readiness.error?[]:readiness.data?.overdue_tasks??[]} risks={readiness.error?[]:readiness.data?.risks??[]}/>
  <ProjectChangeImpact/><HomeDependencyBlockers edges={readiness.data?.task_dependencies} error={readiness.error}/></div><aside className="home-sidebar"><MentorReviewQueue/><EventRobotChecks/><PurchaseApprovalAging data={readiness.data?.approval_aging} error={readiness.error}/><HomeTeamRisks data={readiness.data} error={readiness.error}/>
  </aside></div><button className="home-competition-card home-competition-banner" onClick={()=>navigate("/competition")}><span><small>{pick("FRC competition","תחרות FRC")}</small><strong>{pick("Competition center","מרכז התחרות")}</strong><em>{pick("Scouting, live analysis and alliance strategy","סקאוטינג, ניתוח חי ואסטרטגיית בריתות")}</em></span><b>{pick("Open","פתיחה")} →</b></button>
  </main>;
}
