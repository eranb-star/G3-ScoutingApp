...
export default function FrcWorkPage(){
  ...
  return <main className="hub-page work-page work-command-center">
    ...
    <section className={`work-focus-grid${myArea?"":" has-single-card"}`}>{myArea?<button className="work-workspace-card" onClick={()=>navigate(`/projects?subteam=${myArea.key}`)}><span className="frc-area-mark">{myAreas.length>1?myAreas.length:myArea.mark}</span><span><small>{myAreas.length>1?pick("My FRC teams","צוותי ה-FRC שלי"):pick("My subteam","תת־הצוות שלי")}</small><strong>{myAreas.map(area=>pick(area.name,area.nameHe)).join(" · ")}</strong><em>{myProjects.length} {pick("active projects in primary workspace","פרויקטים פעילים במרחב הראשי")} · {myWorkspaceTasks.length} {pick("open tasks","משימות פתוחות")}</em>{workspaceBlockers?<b>{workspaceBlockers} {pick("blockers need attention","חסמים דורשים טיפול")}</b>:null}</span><i>→</i></button>:null}
    ...
    <section className="work-robot-health"><button className={`work-health-card${criticalIssues||serviceAlerts?" has-alert":""}`} onClick={()=>navigate("/robot-reliability")}><span className="work-health-signal"/><span><small>{pick("Robot operations","תפעול הרובוט")}</small><strong>{pick("Robot Health","בריאות הרובוט")}</strong><em>{issues.length} {pick("open issues","תקלות פתוחות")} · {serviceAlerts} {pick("service alerts","התראות שירות")}</em></span><b>{criticalIssues?pick("ATTENTION","דורש טיפול"):pick("OPEN","פתיחה")} →</b></button>
      <div className="work-quick-tools"><button onClick={()=>navigate("/robot-issues")}><span>FIX</span><strong>{pick("Issues","תקלות")}</strong></button><button onClick={()=>navigate("/robot-maintenance")}><span>LIFE</span><strong>{pick("Maintenance","תחזוקה")}</strong></button></div></section>

    <section className="work-destination-section work-departments-group"><header><div><small>{pick("FRC departments","תחומי FRC")}</small><h2>{pick("Go directly to a team workspace","מעבר ישיר למרחב צוות")}</h2></div><button className="work-section-toggle" type="button" aria-expanded={departmentsOpen} aria-controls="frc-department-destinations" onClick={()=>setDepartmentsOpen(value=>!value)}><span>{frcAreas.length+2} {pick("destinations","יעדים")}</span><b aria-hidden="true">⌄</b></button></header><div id="frc-department-destinations" className="work-destination-grid" hidden={!departmentsOpen}>{frcAreas.map(area=>{const count=projects.filter(project=>areaMatches(project.subteam,area.key)).length;return <button key={area.key} onClick={()=>navigate(`/projects?subteam=${area.key}`)}><span>{area.mark}</span><strong>{pick(area.name,area.nameHe)}</strong><small>{count} {pick("active projects","פרויקטים פעילים")}</small><b>→</b></button>})}<button className="work-engineering-hub" onClick={()=>navigate("/engineering")}><span>DEV</span><strong>{pick("Engineering Hub","מרכז הנדסה")}</strong><small>{pick("Robot code, CAD, scouting data and experiments","קוד רובוט, CAD, נתוני סקאוטינג וניסויים")}</small><b>→</b></button><button className="work-all-projects" onClick={()=>navigate("/projects")}><span>ALL</span><strong>{pick("All team projects","כל פרויקטי הקבוצה")}</strong><small>{isAdmin?pick("Portfolio and archive management","ניהול תיק פרויקטים וארכיון"):pick("Team-wide project overview","סקירת פרויקטים קבוצתית")}</small><b>→</b></button></div></section>
