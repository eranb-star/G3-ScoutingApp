...
export default function FrcWorkPage(){
  ...
  return <main className="hub-page work-page work-command-center">
    ...
    <section className={`work-focus-grid${myArea?"":" has-single-card"}`}>{myArea?<button className="work-workspace-card" onClick={()=>navigate(`/projects?subteam=${myArea.key}`)}><span className="frc-area-mark">{myArea.mark}</span><span><small>{pick("My subteam","תת־הצוות שלי")}</small><strong>{pick(myArea.name,myArea.nameHe)}</strong><em>{myProjects.length} {pick("active projects","פרויקטים פעילים")} · {myWorkspaceTasks.length} {pick("open tasks","משימות פתוחות")}</em>{workspaceBlockers?<b>{workspaceBlockers} {pick("blockers need attention","חסמים דורשים טיפול")}</b>:null}</span><i>→</i></button>:null}
