...
const suggestions = [
  ...
  ["Compare mechanism options for our robot", "השוו אפשרויות למנגנון הרובוט שלנו"],
  ["Evaluate a strategy for the current season", "העריכו אסטרטגיה לעונה הנוכחית"],
...
export default function FrcAssistantPage() {
  ...
  return <main className="assistant-page">
    ...
    {BUDGET_PILOT&&<SoftwareMentorContext value={software} onChange={value=>{setSoftware(value);if(value)questionRef.current?.focus();}} locked={!canUseAssist||busy||conversationLoading||!!conversationId||!!pendingRequest}/>}
      <label className="assistant-question-label" htmlFor="assist-question">{software?pick("2. Ask about the selected code","2. שאלו על הקוד שנבחר"):pick("Your question","השאלה שלכם")}</label>
