...
const checks=[
  ...
  ["purchase dashboard exposes clear status filters",read("apps/dashboard_web/src/pages/ToolsInventoryPage.tsx").includes("purchase-dashboard")&&read("apps/dashboard_web/src/pages/ToolsInventoryPage.tsx").includes("purchaseFilter")],
  ["purchase rejection requires an audited reason",read("apps/dashboard_web/src/pages/ToolsInventoryPage.tsx").includes("Reject with reason")&&read("backend/supabase/purchase_audit_dashboard_20260907.sql").includes("A rejection reason is required")],
  ["purchase state changes use the secured transition RPC",read("apps/dashboard_web/src/pages/ToolsInventoryPage.tsx").includes('rpc("transition_purchase_request"')&&read("backend/supabase/purchase_audit_dashboard_20260907.sql").includes("if not public.is_admin()")],
  ["purchase status changes create persistent requester updates",read("backend/supabase/purchase_audit_dashboard_20260907.sql").includes("'purchase_request_status'")&&read("backend/supabase/purchase_audit_dashboard_20260907.sql").includes("request.requested_by::text")],
  ...
  ["calendar managers can edit and logically delete",read("apps/dashboard_web/src/pages/UnifiedCalendarPage.tsx").includes("openEdit(selected)")&&read("apps/dashboard_web/src/pages/UnifiedCalendarPage.tsx").includes('rpc("cancel_team_calendar_event"')&&read("backend/supabase/calendar_event_management_20260906.sql").includes("public.has_permission")],
