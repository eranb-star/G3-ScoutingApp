...
begin
 ...
 select c.id,c.url,s.title,p.body,s.version_hash version,s.seasons,s.scope,s.source_class,s.id source,
 ...
 where not s.retired and p.search@@q
 ...
 ), numbered as (select *,row_number() over(partition by kind order by rank desc,id) position from matched),
 selected as (select * from numbered where position<=case when kind='manual' then 5 else 3 end)
 select coalesce(jsonb_agg(jsonb_build_object('id',id,'url',url,'title',title,'body',body,'version',version,'seasons',seasons,'scope',scope,'source_class',source_class) order by kind,rank desc,id),'[]'::jsonb) from selected);
end$$;
