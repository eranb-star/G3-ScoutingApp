...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const [{data:allMembers},{data:mentionRows}]=await Promise.all([admin.from("team_members").select("id,role,subteam,subteams").eq("active",true).neq("id",user.id),admin.from("channel_message_mentions").select("member_id").eq("message_id",message.id)]);
    const mentionedIds=new Set((mentionRows??[]).map(row=>row.member_id));
    const normalized=(value:string|null)=>{const text=String(value??"").trim().toLowerCase();if(["mechanical","mech"].includes(text))return "mechanical";if(["cad","cad & design","design"].includes(text))return "cad";if(["electrical","electronics","elec"].includes(text))return "electrical";if(["software","code","programming"].includes(text))return "software";if(["strategy","strategy & scouting","scouting"].includes(text))return "strategy";if(["field","field build","field build & infrastructure"].includes(text))return "field";if(["pit","drive & pit","drive team"].includes(text))return "pit";if(["business","business & outreach","outreach"].includes(text))return "business";if(["publicity","publicity & awards","judging","awards"].includes(text))return "publicity";return text;};
    const recipients=(allMembers??[]).filter(member=>channel.kind!=="leadership"||["admin","mentor","team_leader"].includes(member.role)).filter(member=>channel.kind!=="subteam"||[...(member.subteams??[]),member.subteam].some(team=>normalized(team)===normalized(channel.subteam)));let ids=recipients.map(row=>row.id);
    ...
    const results=await Promise.all(tokens.map(async row=>{const hidden=preferenceMap.get(row.member_id)?.private_previews===true;const mentioned=mentionedIds.has(row.member_id),response=await fetch(endpoint,{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify({message:{token:row.token,notification:{title:hidden?"New G3 Team Hub message":mentioned?`${author.display_name} mentioned you in #${channel.name}`:`#${channel.name} · ${author.display_name}`,body:hidden?"Open the app to view it.":message.body.slice(0,180)},android:{priority:mentioned?"HIGH":"NORMAL"},data:{path:`/updates?view=channels&channel=${message.channel_id}&message=${message.id}`,channelId:message.channel_id,messageId:message.id}}})});if(response.ok)return true;const failure=await response.text();if(response.status===404||failure.includes("UNREGISTERED")||failure.includes("INVALID_ARGUMENT"))await admin.from("push_tokens").update({active:false,updated_at:new Date().toISOString()}).eq("token",row.token);return false;}));
    return new Response(JSON.stringify({delivered:results.filter(Boolean).length,failed:results.filter(result=>!result).length,recipients:ids.length}),{headers:{...cors,"Content-Type":"application/json"}});
