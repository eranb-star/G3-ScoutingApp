...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const {data:allMembers}=await admin.from("team_members").select("id,role,subteam").eq("active",true).neq("id",user.id);
    ...
    const {data:preferenceRows}=await admin.from("notification_preferences").select("member_id,channel_messages,private_previews").in("member_id",ids);const preferenceMap=new Map((preferenceRows??[]).map(row=>[row.member_id,row]));ids=ids.filter(id=>preferenceMap.get(id)?.channel_messages!==false);
