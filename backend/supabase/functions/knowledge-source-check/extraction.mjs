...
export async function extractPdf(data, getDocumentProxy) {
  ...
  try {
    ...
    if(!chunks.length||empty>Math.max(3,pdf.numPages*.3))throw Error('PDF has insufficient extractable text. Scanned/image pages need OCR or manual review; no partial index was published.');
    return {chunks,pages:pdf.numPages,emptyPages:empty,format:'pdf'};
